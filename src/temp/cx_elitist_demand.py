def cx_operator(input_ind1, input_ind2, env):
    try:
        ...
        # Precompute node demand status and edge visual quality map for O(1) access
        ...
        for u, v in env.G.edges():
            ...
            edge_visual_ratio[(u, v)] = (pi - ni) / (length + 1e-6)
            edge_visual_ratio[(v, u)] = edge_visual_ratio[(u, v)]

        # Threshold for weak link detection — now demand-biased, not visual-only
        threshold_demand = 0.0  # Weak links are *only* those with zero demand at both ends AND low visual quality
        ...
        def identify_building_blocks(route):
            ...
            every edge is either (a) visually strong OR (b) incident to at least one high-demand node.
            ...
            while i < len(route):
                ...
                # Edge qualifies if visually strong OR touches demand node
                visual_ratio = edge_visual_ratio.get((u_int, v_int), -1.0)
                demand_u = node_demand.get(u_int, 0)
                demand_v = node_demand.get(v_int, 0)
        ...
        def select_cut_points_from_blocks(route, blocks):
            ...
            for i in range(len(blocks) - 1):
                ...
                # Candidate cut index = end_of_block + 1 (start of next block)
                cut_idx = end_of_block + 1
                if cut_idx < len(route) - 1:
                    # Prioritize cuts where the *preceding* node (end_of_block) is low-demand & has weak outgoing edges
                    prev_node = route[end_of_block]
                    prev_int = int(prev_node)
                    is_weak_endpoint = node_demand.get(prev_int, 0) == 0
                    # Also check if next edge is weak (to target weak-link endpoint)
                    if cut_idx < len(route):
                        next_node = route[cut_idx]
                        next_int = int(next_node)
                        if is_weak_edge(prev_int, next_int):
                            candidates.insert(0, cut_idx)  # High-priority
                        else:
        ...
        def repair_path_segment(start_node, end_node):
            ...
            Uses precomputed maps & bounded Dijkstra."""
            ...
            try:
                def weight_func(u_, v_, d):
                    ...
                    # Strongly penalize non-demand edges; reward demand-adjacent edges
                    base_weight = length / (1e-6 + max(visual_ratio, 0.0) + 1e-6)
                    demand_bonus = 0.0
                    if demand_u == 1 or demand_v == 1:
                        demand_bonus = -1.5 * length  # Large incentive
                    elif demand_u == 0 and demand_v == 0:
                        demand_bonus = +0.8 * length  # Penalty
                    
                    return base_weight + demand_bonus + 0.002 * flow_val

                # Bounded Dijkstra: limit search depth to avoid blowup
                ...
                while pq and len(visited) < max_nodes_explored:
                ...
                # Fallback: simple shortest path by distance
        ...
        for i in range(num_routes):
            ...
            # Enforce depot anchoring *before* splicing: ensure routes start/end at depots
            ...
            # Identify building blocks and *weak-link-biased* cut points
            ...
            # Pre-repair feasibility check: prioritize demand satisfaction & load balance
            ...
            # Demand-balanced acceptance: allow slight drop only if improves balance (reduces max)
            ...
            # Accept if demand coverage preserved *or* imbalance reduced (Pareto improvement on Objective 2)
            accept1 = (new_demand1 >= min_demand) or (new_max_demand < max_demand)
            accept2 = (new_demand2 >= min_demand) or (new_max_demand < max_demand)

            if accept1:
                try:
                    # Repair *only* the junction, using demand-biased path
